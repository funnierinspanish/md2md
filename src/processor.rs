...
mod tests {
    ...
    fn test_process_single_file_with_includes() {
        ...
        let source_file = temp_dir.path().join("source.md");
        fs::write(&source_file, "!include (header.md)\n\nMain content.").expect("Failed to write source file");
    ...
    fn test_process_single_file_with_failed_include() {
        ...
        assert!(result.error_message.as_ref().expect("Error message should be present for failed includes").contains("failed includes"));
