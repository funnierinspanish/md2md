...
pub fn render(f: &mut Frame, app: &App, area: Rect) {
    let summary = app.summary.lock().expect("Failed to acquire summary lock for summary rendering");
    ...
    let mut content = vec![
        ...
        Line::from(""),
        Line::from(Span::styled("FILES", Style::default().fg(Color::White).bold())),
        Line::from(vec![
            Span::raw("Total: "),
            Span::styled(summary.results.len().to_string(), Style::default().fg(Color::Cyan)),
        ]),
        Line::from(vec![
            Span::raw("Successful: "),
            Span::styled(successful_files.to_string(), Style::default().fg(Color::Green)),
        ]),
        Line::from(vec![
            Span::raw("Failed: "),
            Span::styled(failed_files.to_string(), Style::default().fg(if failed_files > 0 { Color::Red } else { Color::Green })),
        ]),
        Line::from(""),
        Line::from(Span::styled("INCLUDES", Style::default().fg(Color::White).bold())),
        Line::from(vec![
            Span::raw("Total: "),
        ...
        Line::from(vec![
            Span::raw("Successful: "),
            Span::styled(successful_includes.to_string(), Style::default().fg(Color::Green)),
        ]),
        Line::from(vec![
            Span::raw("Failed: "),
            Span::styled(failed_includes.to_string(), Style::default().fg(if failed_includes > 0 { Color::Red } else { Color::Green })),
        ]),
    ];
    ...
    if app.processing_complete && !summary.results.is_empty() {
        ...
        content.push(Line::from(Span::styled("PERFORMANCE", Style::default().fg(Color::White).bold())));
        content.push(Line::from(vec![
            Span::raw("Avg time per file: "),
            Span::styled(format!("{:.2}ms", avg_time_per_file), Style::default().fg(Color::Cyan)),
        ]));
        ...
        if total_includes > 0 {
            ...
            content.push(Line::from(vec![
                ...
                Span::styled(format!("{:.2}ms", avg_time_per_include), Style::default().fg(Color::Cyan)),
            ]));
        }
    }

    let summary_widget = Paragraph::new(content)
        .block(Block::default().borders(Borders::ALL).title("Processing Summary"))
        .wrap(Wrap { trim: true });
    f.render_widget(summary_widget, chunks[0]);

    // Recent Activity (last few processed files)
    let recent_activity: Vec<Line> = summary.results
        .iter()
        .rev()
        .take(5)
        .map(|result| {
