...
pub fn write_file(path: &Path, content: &str) -> Result<(), Box<dyn std::error::Error>> {
    ensure_output_directory(path).expect("Failed to ensure output directory exists");
    fs::write(path, content).expect("Failed to write file content");
    Ok(())
}
...
mod tests {
    ...
    fn test_collect_markdown_files_directory() {
        ...
        fs::write(dir_path.join("not_markdown.txt"), "Not markdown")
        ...
        assert!(
            files
                .iter()
                .any(|f| f.file_name().expect("Failed to get filename") == "file1.md")
        );
        assert!(
            files
                .iter()
                .any(|f| f.file_name().expect("Failed to get filename") == "file2.md")
        );
        assert!(
            files
                .iter()
                .any(|f| f.file_name().expect("Failed to get filename") == "file3.md")
        );
    }
    ...
    fn test_write_file_overwrite() {
        ...
        assert_eq!(
            fs::read_to_string(&file_path).expect("Failed to read initial content"),
            "Initial content"
        );
        ...
        assert_eq!(
            fs::read_to_string(&file_path).expect("Failed to read new content"),
            "New content"
        );
    }
}
